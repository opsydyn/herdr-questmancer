...
def page(number, title, subtitle, height):
    ...
    text(d,(48,28),'QUESTMANCER  /  THREE-CLASS PILOT  /  '+number,15,GOLD,'mono')
...
im,d=page('01','Three adventurers, eight authored moments','Production 16 × 24 frames. Stable feet on row 21; persona colours applied by the existing palette system.',1330)
...
text(d,(48,1002),'Persona variation and authored roster partners',25,GOLD,'serif')
for index in range(12):
    x=48+index*92;put(im,f'variant-{index}',x,1060,4)
for index in range(3):
    x=1170+index*80;put(im,f'{index}-roster',x,1090,5);text(d,(x,1166),NAMES[index][:3],14,MUTED)
text(d,(48,1224),'Book / map / tool remain class-specific. The shared raised signal settles into a calm blocked pose.',18)
...
im,d=page('03','A brief gesture, then a quiet room','Production room crops: Wizard, Ranger and Barbarian in separate fixtures, each at its real station.',1930)
def actor_strip(key,scale=4):
    # Show each pilot separately so the Delve's two-per-station limit cannot hide a class.
    out=Image.new('RGB',(120*scale,44*scale),BG)
    for index in range(3):
...
def playback(keys,label):
    ...
    for col,(world,key) in enumerate(zip(['hall','delve'],keys)):
        ...
        text(draw,(x+15,130),'Wizard             Ranger             Barbarian',15,GOLD,'mono')
...
# A small native-file proof for unchanged class-card layouts and the updated Delve golden.
proof=Image.new('RGB',(1800,680),BG);draw=ImageDraw.Draw(proof)
for index in range(3):
...
text(draw,(550,460),'Exact quiet Delve golden fixture / updated production art',22)
text(draw,(550,500),'All artwork above uses production paths. These are fixture exports.',20,MUTED)
