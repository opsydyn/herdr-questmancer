...
fn persona(class: AdventurerClass) -> AdventurerPersona {
    ...
    p.appearance.hair_tone = match class {
        AdventurerClass::Cleric => HairTone::Silver,
        AdventurerClass::Druid => HairTone::Silver,
        _ => HairTone::Chestnut,
    };
    p
}
...
fn main() {
    ...
    for c in input["classes"].as_array().unwrap() {
        ...
        let class = match id {
            "cleric" => AdventurerClass::Cleric,
            "paladin" => AdventurerClass::Paladin,
            "druid" => AdventurerClass::Druid,
            _ => unreachable!(),
        };
