...
def terminal(data):
 ...
 return im
im=Image.new('RGB',(1540,1150),BG);d=ImageDraw.Draw(im)
text(d,(30,24),'Chronicle chapters / recorded events, retained sources',28,GOLD)
text(d,(30,74),'Tab requests a last-hour guild chapter; Tab returns to the existing selected record list.',16,MUTED)
for i,(key,title) in enumerate([('records','c / existing selected-adventurer records'),('chapter','Tab / last-hour guild chapter'),('sources','j / source entries below the recap'),('empty','A later window / no retained events')]):
 x=30+(i%2)*770;y=126+(i//2)*492
 text(d,(x,y),title,18,GOLD);im.paste(terminal(DATA[key]),(x,y+35))
text(d,(30,1100),'Production Ratatui reconstruction. Fixed fixture names and times; visual review pending.',16,MUTED)
im.save(ROOT/'chronicle-chapters.png')
