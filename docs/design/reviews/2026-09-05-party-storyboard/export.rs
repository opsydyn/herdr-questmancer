...
fn persona(class: AdventurerClass) -> AdventurerPersona {
    ...
    p.appearance.hair_tone = match class {
        AdventurerClass::Wizard => HairTone::Silver,
        AdventurerClass::Barbarian => HairTone::Copper,
        _ => HairTone::Chestnut,
    };
    p
}
...
fn main() {
    ...
    for c in input["classes"].as_array().unwrap() {
        ...
        let class = match id {
            "wizard" => AdventurerClass::Wizard,
            "ranger" => AdventurerClass::Ranger,
            "barbarian" => AdventurerClass::Barbarian,
            _ => unreachable!(),
        };
        ...
        for (name, rows) in c["frames"].as_object().unwrap() {
            ...
            assert_ne!(f, before, "{id} {name}: unchanged production master");
            assert!(
        ...
        assert_ne!(
            ...
            "working frames must alter pixels"
        );
        let roster = frame(&c["roster"], &palette);
        assert_eq!(roster.size(), PixelSize::new(8, 12));
        assert_eq!(foot(&roster).0, 10);
        ...
        println!(
            "{id}: 8 world frames 16x24; foot line 21 and horizontal anchor stable; authored roster 8x12"
        );
    }
    let marker_palette = [
        ('o', "e6cf9a"),
    ...
    for (name, world, view) in [
        ...
        // Three fixed study personas and real state labels, in the existing fixture topology.
        for (i, key) in keys.iter().enumerate() {
            ...
            if i < 3 {
                ...
                a.name = format!("{:?}", class);
                a.presence = [Presence::Working, Presence::Blocked, Presence::Idle][i];
