...
IDS = ['rogue', 'pathseeker', 'runewright']
...
def page(number, title, subtitle, height):
    ...
    text(d, 44, 24, 'QUESTMANCER  /  LOCK, COMPASS & RUNE STUDIES  /  ' + number, 15, GOLD)
...
im, d = page('01', 'A lock, a compass, a rune stone', 'Larger heads, shorter bodies, planted feet. Class gear carries the identity.', 1170)
notes = {
'rogue': ['Purple hood and a low face wrap.', 'Paired daggers stay sheathed at the belt.', 'A tiny pick turns in a brass practice lock.'],
'pathseeker': ['A green scarf inside a travel hood.', 'The green lantern hangs from its staff.', 'A thumb adjusts the compass rim.'],
'runewright': ['Warm brown hood; a broad white beard.', 'A stone tablet and a short square mallet.', 'The mallet taps the stone’s edge.']}
for i, slug in enumerate(IDS):
    x = 44 + i * 484; c = DATA['classes'][slug]
...
im, d = page('02', 'Small hands, distinct rituals', 'Authored 16×24 frames shown at 4×. State markers stay readable when all motion is off.', 1120)
...
text(d,44,910,'Resume only when Herdr reports working. Sending counsel does not restart the work loop.',20)
text(d,44,948,'Spoils are a sealed lock case, folded route folio or bound stone: a completion metaphor, never verified contents.',18,MUTED)
text(d,44,980,'Fresh spoils settle within three seconds. Reduced/still use static poses; exited adventurers leave the scene.',18,MUTED)
text(d,44,1012,'The shared lantern and completion / unknown marks keep their existing meanings.',18,MUTED)
...
text(d,44,985,'Rogue working · Pathseeker seeking counsel · Runewright resting. No live Herdr or native-terminal acceptance claimed.',17,MUTED)
...
(ROOT/'verification.json').write_text(json.dumps({'scope':'Design candidates; no production routing or native acceptance','visual_approval':'pending','dimensions':'All 24 authored frames are 16x24 with identical foot anchors per class','checks':'Rust exporter validates palette, frame bounds, feet, nonempty frames and working pixel changes','output_sha256':outputs},indent=2)+'\n')
