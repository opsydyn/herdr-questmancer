...
fn counsel(outcome: &str, ascii: bool) -> Model {
    let mut m = party(View::Guild, Presence::Blocked, 1_000, Motion::None, 2);
...
fn main() {
    ...
    let approved: Value = serde_json::from_str(
        &fs::read_to_string(
            std::path::Path::new(&directory)
                .join("../2026-09-08-mage-sorcerer-storyboard/candidates.json"),
        )
        .unwrap(),
    )
    .unwrap();
    ...
    println!(
        "16 production poses match the approved silhouettes and class-owned material pixels; persona roles remain dynamic."
    );
    for index in 0..12 {
        data.insert(
    ...
    for (world, view) in [("hall", View::Guild), ("delve", View::Delve)] {
        for (label, presence, age) in [
            ...
            let m = party(view, presence, age, Motion::Full, 2);
            data.insert(
                format!("{world}-{label}"),
        ...
        for frame in 0..25 {
            let m = party(view, Presence::Done, frame * 125, Motion::Full, 2);
            data.insert(
