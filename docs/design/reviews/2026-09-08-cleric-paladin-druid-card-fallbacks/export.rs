...
use questmancer::{
    ...
    scene::{
        assets::{
            adventurer::{
                adventurer_animation_frame, adventurer_portrait_frame, druid_portrait_frame,
            },
            archetypes,
        },
...
fn main() {
    ...
    for (index, class) in CLASSES.into_iter().enumerate() {
        ...
        data.insert(
            format!("before-{index}"),
            frame_json(&if class == AdventurerClass::Druid {
                druid_portrait_frame()
            } else {
                archetypes::portrait_frame(class).unwrap()
            }),
        );
        data.insert(
            format!("after-{index}"),
        ...
        a.persona = p;
        a.name = ["Aria", "Tobin", "Mira"][index].to_owned();
