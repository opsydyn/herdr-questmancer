...
- Reused `compose_seated_for_palette` and `pack` for the palette-safe 10x12
  seated persona. Exited agents render an empty-chair/broken-CRT scene.
...
- `cargo test --all-targets` — passed, including all 11 `cafe_widgets` tests
...
- The widget does not infer focus from selection. Selection changes the border
  and cursor; `TheatreFrame::focused` independently controls the lamp and
  `LIVE`, leaving Task 5 free to define cafe selection/focus coordination.
